...
bool Teacher::update_book_details(Library& library, string title, string new_author, string new_genre, string new_year_published) {
    // Get the list of books from the library
    vector<Book*>& books = library.getBooks();

    // Iterate through the books to find a match
    for (Book* book : books) {
        if (book->getTitle() == title) {
            // Update book details
            book->setAuthor(new_author);
            book->setGenre(new_genre);
            book->setYearPublished(new_year_published);

            // Print success message
            cout << "Book details updated for title: " << title << endl;
